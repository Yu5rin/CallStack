...
export interface Settings {
  ...
  /**
   * 新しい版を見つけたら、ダウンロードまで自動で済ませておく（Windows限定。既定OFF）。
   * ON でも実際にアプリを再起動して適用するのは、通知をクリックするか設定画面のボタンを
   * 押したときだけ（記録中は適用されない）。OFF でも「今すぐ更新」自体は行える
   * （更新できるかどうかはこのトグルに左右されない。opt-in はダウンロードのタイミングのみ）。
   */
