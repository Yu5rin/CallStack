...
export function LiveApp() {
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-slate-900 text-slate-100">
      ...
      <div className="hud-drag flex flex-none select-none items-center gap-1.5 border-b border-white/10 bg-slate-950/60 px-2 py-1">
        <AudioLines size={12} strokeWidth={2.25} className="flex-none text-sky-300" />
        <span className="text-[10px] font-semibold text-slate-300">ライブ字幕</span>
        <div className="hud-no-drag ml-auto flex items-center gap-0.5">
          {(['sm', 'md', 'lg'] as const).map((s) => (
            <button
              ...
              className={`rounded px-1.5 py-0.5 text-[10px] font-semibold transition ${
                fontSize === s ? 'bg-sky-500 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-slate-100'
              }`}
          ...
          <button
            onClick={handleClose}
            className="ml-1 rounded p-0.5 text-slate-400 transition hover:bg-red-500 hover:text-white"
      ...
      <div className="relative min-h-0 flex-1">
        ...
        >
          {!hasContent ? (
            ...
            <>
              {lines.map((t, i) => (
                <div key={i} className="whitespace-pre-wrap break-words text-slate-100">{t}</div>
              ))}
              {partial && (
                <div className="whitespace-pre-wrap break-words italic text-slate-400">{partial}</div>
              )}
            </>
          )}
        </div>
        {!autoFollow && (
          <button
            ...
            className="hud-no-drag absolute bottom-2 right-3 inline-flex items-center gap-0.5 rounded-full bg-sky-500 px-2.5 py-1 text-[11px] font-semibold text-white shadow-lg hover:bg-sky-600"
          >
