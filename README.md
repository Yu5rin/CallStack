...
- **議事録（Markdown）の書き出し** — タイトル・参加者・メモ・マーカー・文字起こしをまとめて保存
...
- **録音の安定性** — 録音は専用の不可視ウィンドウで実行され、メインウィンドウの最小化・復帰の影響を受けません
- **通話中の HUD** に経過時間・録音状態・リアルタイム**レベルメータ**を表示
- **累積記録** を JSON で永続化＋日次バックアップ
- **CSV の書き出し・取り込み**（UTF-8 + BOM で Excel 対応、id で upsert）
- **通話時間サマリー**（合計／平均／最長／最短／今日・今週・今月）
- **統計ダッシュボード**（過去30日の棒グラフ、タグ別の横棒グラフ、時間帯別ヒストグラム、連絡先別 TOP10）
...
| 内容 | パス |
|------|------|
| 設定・記録 | `%APPDATA%\CallStack\data.json` |
...
- **保管期限**: 既定 90 日。期限切れの音声ファイルのみ削除（記録は残ります）
