...
- **自動更新（実験的・Windows限定・既定オフ）** — 設定で有効にすると、新バージョンをバックグラウンドでダウンロード・SHA256検証・展開まで自動で行い、ボタン1つで再起動して適用できます（記録中は適用されません。詳細は [自動更新について](#自動更新について) を参照）
...
CallStack は既定では**通知のみ**です（起動時に GitHub の最新リリースを確認し、新しい版があればクリックでダウンロードページを開くだけ）。通話・録音・文字起こしなどのデータは一切送信されません。

設定 → バージョン情報 →「自動更新（実験的・Windows限定）」を ON にすると、新バージョンの検知時に以下まで自動で行われます。

1. GitHub Releases から Windows 用 ZIP をダウンロード
2. **SHA256 で検証**（GitHub が算出した digest と突き合わせ、改ざん・破損を検知）
3. アプリのインストール先フォルダの隣に展開して準備完了

ただし**実際にファイルを入れ替えて再起動するのは、通知またはボタンをクリックしたときだけ**です。記録が進行中の場合は適用を拒否します。適用時は、実行中の exe を直接上書きできない Windows の制約に対応するため、以下の手順で安全に入れ替えます（Chrome・VS Code 等と同様の方式）。
...
6. 前回の更新で残った `.old` フォルダは、次回起動時に自動で削除
